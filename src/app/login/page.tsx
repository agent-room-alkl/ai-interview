  ...
  return (
    <main className="min-h-dvh bg-[#f6f5f0] px-4 py-8 text-[#17201e] sm:px-6">
      ...
      <div className="mx-auto mt-12 max-w-md sm:mt-20">
        ...
        <h1 className="mt-4 text-4xl font-semibold tracking-[-0.07em] sm:text-5xl">Log in.</h1>
        <p className="mt-4 text-[#65736d]">Pick up where your interview practice left off.</p>
        <form onSubmit={submit} className="mt-8 space-y-5 rounded-3xl border border-[#17201e]/10 bg-white/60 p-5 sm:mt-10 sm:p-8">
          {params.get("created") && <p className="rounded-xl bg-[#e3eee7] px-4 py-3 text-sm text-[#356653]">Account created. You can log in now.</p>}
          <label className="block text-sm font-medium">Email<input name="email" type="email" defaultValue={params.get("email") ?? ""} required autoComplete="email" className="mt-2 min-h-11 w-full rounded-xl border border-[#17201e]/15 bg-white px-4 py-3 text-base outline-none focus:border-[#e57b4f] sm:text-sm" /></label>
          <label className="block text-sm font-medium">Password<input name="password" type="password" required autoComplete="current-password" className="mt-2 min-h-11 w-full rounded-xl border border-[#17201e]/15 bg-white px-4 py-3 text-base outline-none focus:border-[#e57b4f] sm:text-sm" /></label>
          {error && <p className="rounded-xl bg-[#fce7df] px-4 py-3 text-sm text-[#9c482d]">{error}</p>}
          <button disabled={pending} className="min-h-12 w-full rounded-full bg-[#17201e] px-5 py-3.5 text-sm font-semibold text-[#f6f5f0] disabled:opacity-50">{pending ? "Logging in..." : "Log in"}</button>
        </form>
